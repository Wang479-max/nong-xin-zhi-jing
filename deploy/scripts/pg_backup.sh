...
dump_file="$backup_dir/${PG_BACKUP_DATABASE}.dump"
mkdir -p "$backup_dir"

pg_dump --format=custom --compress=9 --no-owner --no-privileges --host="$PG_BACKUP_HOST" --port="$PG_BACKUP_PORT" --username="$PG_BACKUP_USER" --file="$dump_file" "$PG_BACKUP_DATABASE"
...
if [[ -n "$COS_BUCKET" ]]; then
  ...
  coscmd upload "$dump_file" "/${COS_REMOTE_PREFIX}/${stamp}/${PG_BACKUP_DATABASE}.dump"
  coscmd upload "$dump_file.sha256" "/${COS_REMOTE_PREFIX}/${stamp}/${PG_BACKUP_DATABASE}.dump.sha256"
fi
