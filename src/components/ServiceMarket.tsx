...
const ServiceMarket: React.FC<Props> = ({ session, onSessionChange }) => {
  ...
  return <div className="mx-auto max-w-7xl p-3 sm:p-6">
    ...
    <div className="mobile-scroll-row mb-6 flex gap-2"><button onClick={() => setTab('catalog')} className={`min-h-11 shrink-0 rounded-xl px-4 py-2 text-sm ${tab === 'catalog' ? 'bg-emerald-500 text-white' : 'bg-white text-slate-600 dark:bg-slate-800 dark:text-slate-300'}`}><Package className="mr-2 inline" size={15} />产品目录</button><button onClick={() => setTab('orders')} className={`min-h-11 shrink-0 rounded-xl px-4 py-2 text-sm ${tab === 'orders' ? 'bg-emerald-500 text-white' : 'bg-white text-slate-600 dark:bg-slate-800 dark:text-slate-300'}`}><ReceiptText className="mr-2 inline" size={15} />我的订单</button></div>
    ...
    {loading ? <div className="flex justify-center py-24"><Loader2 className="animate-spin text-emerald-500" size={32} /></div> : tab === 'catalog' ? (
      <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">{products.map((product) => {
        ...
        return <article key={product.id} className="flex flex-col rounded-2xl border border-slate-200 bg-white p-4 sm:p-6 dark:border-slate-700 dark:bg-slate-800/60">
          ...
          <button disabled={!product.enabled || current || busy !== null} onClick={() => void checkout(product)} className="min-h-11 rounded-xl bg-emerald-500 py-2.5 text-sm font-medium text-white disabled:cursor-not-allowed disabled:opacity-50">{busy === product.id ? <Loader2 className="mx-auto animate-spin" size={16} /> : !product.enabled ? '已停用' : current ? '当前套餐' : '创建订单'}</button>
        </article>;
      })}</div>
    ) : orders.length === 0 ? <div className="py-20 text-center text-slate-400">暂无服务器订单记录</div> : <div className="space-y-3">{orders.map((order) => <article key={order.id} className="flex flex-col items-start justify-between gap-3 rounded-2xl border border-slate-200 bg-white p-4 sm:flex-row sm:items-center dark:border-slate-700 dark:bg-slate-800/60"><div className="min-w-0"><div className="font-medium text-slate-800 dark:text-white">{products.find(({ id }) => id === order.productId)?.name ?? order.productId} × {order.quantity}</div><div className="break-all text-xs text-slate-400">{order.id} · {new Date(order.createdAt).toLocaleString('zh-CN')}</div></div><div className="text-left sm:text-right"><div className="font-bold">{yuan(order.amountFen)}</div><span className={`inline-flex items-center gap-1 text-xs ${order.status === 'paid' ? 'text-emerald-600' : 'text-amber-600'}`}>{order.status === 'paid' ? <Check size={13} /> : <Clock size={13} />}{order.status}</span></div></article>)}</div>}
  </div>;
};
